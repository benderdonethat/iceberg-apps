export default async function handler(req, res) {
  ...
  const AUDIENCE_ID = process.env.RESEND_AUDIENCE_ID; // create once in Resend dashboard

  try {
    // 1. Add to Resend audience as a contact
    if (AUDIENCE_ID) {
      await fetch(`https://api.resend.com/audiences/${AUDIENCE_ID}/contacts`, {
        ...
        body: JSON.stringify({
          email,
          unsubscribed: false,
          // Tags: subscriber, not yet an active app user
          first_name: '',
          last_name: '',
        }),
      });
        ...
        html: `
          <div style="background:#0a0c12;padding:40px 24px;font-family:-apple-system,Helvetica,Arial,sans-serif;">
            <div style="max-width:500px;margin:0 auto;">
              <h1 style="color:#F0F0F5;font-size:22px;font-weight:700;margin-bottom:16px;">You're in.</h1>
              <p style="color:#6b7d8d;font-size:15px;line-height:1.6;margin-bottom:24px;">
                You'll hear from us when we ship something new — new apps, major updates, and things worth knowing about. Nothing else.
              </p>
