/**
 * Newsletter Sender — handles two newsletters:
 *   1. The weekly freeslackapps Superhuman-style digest (default)
 *   2. The Tip (personal brand observation newsletter) — when ?source=tip or body.source === 'tip'
 *
 * Default mode: POST with x-admin-key, generates digest, sends to RESEND_AUDIENCE_ID
 * Tip mode: GET (cron) or POST with ?source=tip, sends CURRENT Tip issue to TIP_AUDIENCE_ID
 */

import { CURRENT as TIP_CURRENT } from './_tip-issues.js';

const TIP_AUDIENCE_ID = '72a0e8a2-24bd-4dab-9f17-641eb20878dd';

export default async function handler(req, res) {
  // THE TIP branch (cron or manual)
  const isTipCron = (req.query?.source === 'tip') || (req.body?.source === 'tip') || req.headers['x-vercel-cron'] === '1';
  if (isTipCron) return handleTipSend(req, res);

  // Default newsletter (freeslackapps digest)
...
/**
 * Send current Tip issue to every subscriber in THE TIP audience.
 * Triggered by Vercel cron (Thursday 10am EDT) via /api/newsletter?source=tip
 * Or manually via POST with { source: 'tip' } + admin key.
 */
async function handleTipSend(req, res) {
  ...
  try {
    ...
    const contacts = (contactsData.data || []).filter(c => !c.unsubscribed);
    if (contacts.length === 0) {
      return res.status(200).json({ success: true, sent: 0, issue: TIP_CURRENT.number, message: 'no subscribers' });
    }
    ...
    return res.status(200).json({ success: true, issue: TIP_CURRENT.number, subject: TIP_CURRENT.subject, sent, failed, total: contacts.length });
  } catch (err) {
